...
class misc(commands.Cog):
    ...
    @commands.slash_command(description="Pings Discord's API")
    async def botping(self, ctx: discord.ApplicationContext):
        embed = discord.Embed(
            title="Ping",
            description=f"The ping to discord is: {round(self.bot.latency * 100, 2)} ms")
        await ctx.respond(embed=embed)

    @commands.slash_command(description="Say cheese!")
    async def cheese(self, ctx: discord.ApplicationContext):
        ...
        embed.set_footer(text="Courtesy of GitHub Copilot")
        await ctx.respond(embed=embed)

    @commands.slash_command(description="Use this when someone says something that is pointless")
    async def pointless(self, ctx: discord.ApplicationContext):
        await ctx.respond("This is literally pointless, why did you do this?")

    @commands.slash_command(debug_guilds=debug_guilds, description="Test!")
    async def kill(self,
    ...
    async def test(self, ctx: discord.ApplicationContext):
        await ctx.respond("Test! This works!!")
